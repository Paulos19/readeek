...
import { uploadCommunityFile } from "@/app/actions/communityUploadActions"; // A action "pesada" vive aqui
...
export default function FileUploadClient({ communityId }: { communityId: string }) {
    ...
    const handleAction = async (formData: FormData) => {
        ...
        if (result?.error) {
            ...
            toast.success("Sucesso!", { description: "Ficheiro enviado." });
    ...
    return (
        <form ref={formRef} action={handleAction} className="mb-6">
            <Card>
                <CardHeader><CardTitle className="text-lg">Adicionar Novo Material</CardTitle></CardHeader>
                <CardContent className="flex flex-col sm:flex-row gap-4 items-center">
                    ...
                    <Button type="submit" size="sm" className="w-full sm:w-auto">
                        <FileUp className="w-4 h-4 mr-2" />Enviar
                    </Button>
                </CardContent>
            </Card>
        </form>
